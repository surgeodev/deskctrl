...
| **🖱️ Full input forwarding** | Keyboard, mouse (move/click/scroll), clipboard. |
...
| **🖼️ GUI + CLI** | Full graphical interface (PyQt6) and command-line. |
...
- ✅ Installe toutes les dépendances (PyQt6, zeroconf, OpenCV...)
- ✅ Crée la commande `deskctrl` accessible dans le terminal
...
pip install 'deskctrl[all]'           # Tout (GUI + découverte LAN)
```
...
git clone https://github.com/deskctrl/deskctrl.git
cd deskctrl
bash scripts/install.sh               # Recommandé (auto)
# Ou manuellement :
python3 -m venv venv
...
### 1️⃣ Start the server (the machine being controlled)
...
### 2️⃣ Connect as client (the machine doing the controlling)
...
### 4️⃣ Scan for servers

```bash
deskctrl scan
```

### 5️⃣ HDMI capture mode
